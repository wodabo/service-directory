 ...
 *
 * HTTP 探测失败但端口仍在监听时返回 `port-open`，而不是 `offline`：
 * 这多半是数据库/中间件被填了一个 http:// 地址（postgres、redis 不讲 HTTP），
 * 进程明明是活的，报成「离线（进程可能挂了）」并套红框是错的。
 */
export async function checkTarget({ url, host, port }, timeout = 3000) {
  if (url) {
    ...
    try {
      ...
      const ms = Date.now() - started;
      const name = err?.name || '';
      result = name === 'TimeoutError' || name === 'AbortError'
        ? { status: 'timeout', latency: ms, via: 'http' }
        : { status: 'offline', latency: ms, via: 'http' };
    }
    // HTTP 不通时回退一次 TCP：端口通说明进程没挂，只是这个端口不讲 HTTP
    if (port) {
      const tcp = await checkTcp(host, port, Math.min(timeout, 2500));
      if (tcp.status === 'online') {
        return { status: 'port-open', latency: tcp.latency, via: 'tcp', httpFailed: true };
      }
    }
    return result;
  }
...
export async function checkTargetWithExternal(target, externalHost, timeout = 3000) {
  ...
  // 外部通 → 就是在线（port-open 也算通：进程活着，只是那个端口不讲 HTTP）
  const alive = (s) => s === 'online' || s === 'port-open';
  if (alive(external.status)) return { ...external, via: 'external', local };

  // 外部不通但本机活着 → 服务只绑了回环
  if (alive(local.status)) {
